...
class BulkIngestSicarMetaCacheTest(unittest.TestCase):
    def test_refresh_sicar_meta_cache_incrementally_when_relation_is_table(self):
        statements = []
        logs = []
        ...
        with (
            ...
            patch.object(bulk_ingest.time, "monotonic", side_effect=[10.0, 13.1]),
        ):
            bulk_ingest._refresh_sicar_meta_cache(["PRODES_A", "SICAR", "PRODES_A"])

        sql_text = "\n".join(query for query, _params in statements)
        self.assertNotIn("REFRESH MATERIALIZED VIEW landwatch.mv_sicar_meta_active", sql_text)
        function_calls = [
            params
            for query, params in statements
            if "landwatch.refresh_sicar_meta_cache" in query
        ]
        self.assertEqual(function_calls, [(["PRODES_A", "SICAR"],)])
        self.assertTrue(any("kind=cache" in log and "elapsed=3s" in log for log in logs))
        self.assertTrue(any("datasets=PRODES_A,SICAR" in log for log in logs))

    def test_refresh_sicar_meta_uses_materialized_view_fallback_during_transition(self):
        statements = []
        logs = []
        ...
        with (
            ...
            patch.object(bulk_ingest, "exec_sql", side_effect=fake_exec),
            patch.object(bulk_ingest, "log_info", side_effect=logs.append),
            patch.object(bulk_ingest.time, "monotonic", side_effect=[20.0, 24.4]),
        ):
            bulk_ingest._refresh_sicar_meta_cache(["PRODES_A"])

        sql_text = "\n".join(query for query, _params in statements)
        self.assertIn("REFRESH MATERIALIZED VIEW landwatch.mv_sicar_meta_active", sql_text)
        self.assertNotIn("landwatch.refresh_sicar_meta_cache", sql_text)
        self.assertTrue(any("kind=mv" in log and "elapsed=4s" in log for log in logs))

    def test_refresh_mvs_updates_sicar_meta_cache_before_remaining_special_mvs(self):
        order = []
