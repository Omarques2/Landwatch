...
describe('rate-limit helpers', () => {
  it('skips tile and car endpoints from the generic api limiter', () => {
    expect(
      shouldSkipGenericApiRateLimit('/attachments/tiles/a/1/2/3.mvt'),
    ).toBe(true);
    expect(
      shouldSkipGenericApiRateLimit('/attachments/pmtiles/assets/1.pmtiles'),
    ).toBe(true);
    expect(shouldSkipGenericApiRateLimit('/cars/tiles/search/1/2/3.mvt')).toBe(
      true,
    );
